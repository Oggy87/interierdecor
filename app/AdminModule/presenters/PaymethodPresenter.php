 ...
 * @package    MyApplication
 */
class Admin_PaymethodPresenter extends Admin_BasePresenter
{
    ...
    public function handleEdit($id) {
        $unit = $this->fetchRow('paymethod', $id);
        $this['editPaymethodForm']->setDefaults($unit);
        $this['editPaymethodForm']['id']->setValue($unit['id']);
        $this->invalidateControl('edit');
    }
    
    public function renderDefault(){
        $this->template->unitform = $this['paymethodForm'];
        $this->template->editform = $this['editPaymethodForm'];
    }

    protected function createComponentPaymethodGrid($name) {
    ...
    public function gridOperationHandler(SubmitButton $button)
    {
        ...
        $grid = $this->getComponent('paymethodGrid');
    ...
    public function questionDelete($dialog, $params) {
        $unit = $this->fetchRow('paymethod', $params['id']);
        return sprintf('Chcete opravdu smazat metodu platby \'%s\'?', $unit['name']);
    }
    ...
    public function delete($id) {
        try {
            ...
            $this->flashMessage("Způsob platby smazán.", "success");
            $this->invalidateControl("grid");
        } catch(PDOException $exc) {

                $this->flashMessage("Nepodařilo se způsob platby smazat.", "error");
                Debug::log($exc,Debug::ERROR);
  
        }
    }
    
    protected function createComponentPaymethodForm($name) {
        $form = new AppForm($this,$name);
        
        $form->addText('name', 'Název')
                ->setRequired("Vyplňte název způsobu platby.");
        
        $form->addText('price', "Cena")
                ->addRule(Form::FILLED, 'Vyplňte cenu.')
                ->addRule(Form::FLOAT, 'Cena musí být číslo.');
        
        $form->addSubmit('save', 'Uložit')->onClick[] = callback($this, 'savePaymethod');
        //$form->getElementPrototype()->class = 'ajax';
        
        return $form;
    }
    
     protected function createComponentEditPaymethodForm($name) {

        $form = $this->createComponentPaymethodForm($name);
        $form->addHidden('id');

        return $form;
    }
    
    public function savePaymethod($button) {
        $values = $button->getForm()->getValues();
        
        try {
            PaymethodModel::save($values);
            $this->flashMessage("Způsob platby uložen.", "success");
            //if(!$this->isAjax()) 
            $this->redirect('this');
            //$this->invalidateControl("grid");
        } catch(PDOException $exc) {
            $this->flashMessage("Nastala chyba při ukládání způsobu platby.", "error");
            Debug::log($exc, Debug::ERROR);
        }
    }
